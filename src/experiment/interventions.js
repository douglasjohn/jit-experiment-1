...
// --- Fallback bucket: DISABLED - we use AOI-specific interventions only -----------------
// const FALLBACK_ARMS = [
...
/** Safe lookup — never throws, falls back to random AOI from task if AOI ID not provided */
export function getArms(taskId, saLevel, aoiId = null) {
  ...
  // If AOI ID is not provided or not found, pick a random AOI from the task's AOI_HELP
  if (AOI_HELP[taskId]) {
    const availableAois = Object.keys(AOI_HELP[taskId]);
    if (availableAois.length > 0) {
      const randomAoiId = availableAois[Math.floor(Math.random() * availableAois.length)];
      const randomAoiContent = AOI_HELP[taskId][randomAoiId];
      const index = [SA_LEVELS.PERCEPTION, SA_LEVELS.COMPREHENSION, SA_LEVELS.PROJECTION].indexOf(saLevel);
      if (index !== -1) {
        const arm = makeAoiArm(saLevel, randomAoiContent[index]);
        // Add a property to indicate this was a randomly selected AOI
        arm.randomAoiSelection = true;
        arm.selectedAoiId = randomAoiId;
        return [arm];
      }
    }
  }
  
  // Final fallback to bucket system if no AOI-specific content available
  const key = `${taskId}::${saLevel}`;
  if (BANK[key]) return BANK[key];
  
  // If nothing else, return empty array (no intervention)
  return [];
}
